...
function showQuickPanel(context, code) {
    const katexBase = vscode.Uri.joinPath(context.extensionUri, 'node_modules', 'katex', 'dist');
    quickPanelCode = code;
    if (!quickPanel) {
        quickPanel = vscode.window.createWebviewPanel('pythonVisualizerQuick', 'Equation Preview', { viewColumn: vscode.ViewColumn.Beside, preserveFocus: true }, {
            enableScripts: true,
        ...
        quickPanel.webview.onDidReceiveMessage(() => {
            if (quickPanelCode) {
                showExtendedPanel(context, quickPanelCode);
            }
        });
    }
    ...
    const { latex, error } = (0, pythonToLatex_1.pythonToLatex)(code);
...
let extPanel;
function showExtendedPanel(context, code) {
    const katexBase = vscode.Uri.joinPath(context.extensionUri, 'node_modules', 'katex', 'dist');
    if (extPanel) {
        extPanel.reveal(vscode.ViewColumn.Beside);
    }
    else {
        extPanel = vscode.window.createWebviewPanel('pythonVisualizerExtended', 'Expression Visualizer', vscode.ViewColumn.Beside, {
            enableScripts: true,
            localResourceRoots: [katexBase],
            retainContextWhenHidden: true,
        });
...
// ─── Quick panel HTML ─────────────────────────────────────────────────────────
function buildQuickHtml(webview, latex, error, katexJs, katexCss) {
...
// ─── Extended panel HTML ──────────────────────────────────────────────────────
function buildExtendedHtml(webview, code, latex, error, katexJs, katexCss) {
    const nonce = getNonce();
