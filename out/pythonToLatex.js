...
exports.pythonToLatex = pythonToLatex;
// ─── Tokenizer ───────────────────────────────────────────────────────────────
function tokenize(src) {
    ...
    while (i < src.length) {
        ...
        if (ch === '\n') {
            out.push({ type: "NEWLINE" /* TT.Newline */, value: '\n' });
            i++;
            continue;
        }
        if (ch === '#') {
        ...
        if (ch === '"' || ch === "'") {
            ...
            i += q.length;
            out.push({ type: "STRING" /* TT.String */, value: s });
            continue;
        }
        // Numbers
        if (/\d/.test(ch) || (ch === '.' && /\d/.test(src[i + 1] ?? ''))) {
            ...
            if (j < src.length && /[eE]/.test(src[j])) {
                j++;
                if (j < src.length && /[+\-]/.test(src[j]))
                    j++;
                while (j < src.length && /\d/.test(src[j]))
                    j++;
            }
            if (j < src.length && /[jJ]/.test(src[j]))
                j++; // complex
            out.push({ type: "NUMBER" /* TT.Number */, value: src.slice(i, j) });
            i = j;
            continue;
        }
        ...
        if (/[_\p{L}]/u.test(ch)) {
            let j = i;
            while (j < src.length && /[\d_\p{L}]/u.test(src[j]))
                j++;
            out.push({ type: "IDENTIFIER" /* TT.Identifier */, value: src.slice(i, j) });
            i = j;
            continue;
        }
        ...
        if (two === '**') {
            out.push({ type: "**" /* TT.DoubleStar */, value: '**' });
            i += 2;
            continue;
        }
        if (two === '//') {
            out.push({ type: "//" /* TT.DoubleSlash */, value: '//' });
            i += 2;
            continue;
        }
        if (two === '==') {
            out.push({ type: "==" /* TT.EqEq */, value: '==' });
            i += 2;
            continue;
        }
        if (two === '!=') {
            out.push({ type: "!=" /* TT.NotEq */, value: '!=' });
            i += 2;
            continue;
        }
        if (two === '<=') {
            out.push({ type: "<=" /* TT.LtEq */, value: '<=' });
            i += 2;
            continue;
        }
        if (two === '>=') {
            out.push({ type: ">=" /* TT.GtEq */, value: '>=' });
            i += 2;
            continue;
        }
        if (two === '->' || two === ':=') {
        ...
        if (SINGLE[ch]) {
            out.push({ type: SINGLE[ch], value: ch });
            i++;
            continue;
        }
        i++; // skip unknown
    }
    out.push({ type: "EOF" /* TT.EOF */, value: '' });
    return out;
}
...
// ─── Function conversion ──────────────────────────────────────────────────────
function funcToLatex(rawName, args) {
    ...
    switch (name) {
        ...
        case 'trace':
        case 'tr': return `\\operatorname{tr}\\!\\left(${a}\\right)`;
        case 'inv': return `{${a}}^{-1}`;
        case 'pinv': return `{${a}}^{+}`;
        ...
        // Complex
        case 'conj':
        case 'conjugate': return `\\overline{${a}}`;
        ...
        case 'prod':
        case 'product': return `\\prod ${a}`;
        case 'mean':
        case 'average': return `\\bar{${a}}`;
        case 'var':
        case 'variance': return `\\operatorname{Var}\\!\\left(${a}\\right)`;
...
class Parser {
    ...
    skipNewlines() {
        while (this.peek().type === "NEWLINE" /* TT.Newline */)
            this.advance();
    }
    // ── Top-level: one statement (assignment or expression) ────────────────────
    parseStatement() {
        this.skipNewlines();
        if (this.peek().type === "EOF" /* TT.EOF */)
            return '';
    ...
    parsePostfix() {
        ...
        if (this.peek().type === "IDENTIFIER" /* TT.Identifier */) {
            ...
            if (dotted)
                return this.continuePostfix(dotted);
    ...
    parsePrimary() {
        const t = this.peek();
        if (t.type === "NUMBER" /* TT.Number */) {
            this.advance();
            return formatNumber(t.value);
        }
        if (t.type === "STRING" /* TT.String */) {
            this.advance();
    ...
    parseArgList(closing) {
        const args = [];
        while (this.peek().type !== closing && this.peek().type !== "EOF" /* TT.EOF */) {
...
//# sourceMappingURL=pythonToLatex.js.map